...
function buildProviders(): Provider[] {
	...
	const providerNames = process.env.LLM_PROVIDERS?.split(',')
		.map((s) => s.trim())
		.filter(Boolean);
	...
	if (providerNames?.length) {
		...
		for (const name of providerNames) {
			...
			if (!config)
				throw new Error(
					`Unknown LLM provider: ${name}. Available: ${Object.keys(PROVIDER_CONFIGS).join(', ')}`
				);
 ...
 */
export async function llmChatWithProvider(providerName: string, opts: LLMCallOptions): Promise<LLMCallResult> {
	...
	try {
		...
		try {
			response = await provider.client.chat.completions.create(base);
		} catch (err) {
			// Not every OpenAI-compatible endpoint supports json_schema response
			// formats. Downgrade to plain json_object (still parseable) rather than
			// failing the investigation. Runtime zod validation catches the rest.
			if (
				opts.responseFormat &&
		...
		if (err instanceof OpenAI.APIError && (err.status === 429 || err.status === 402 || err.status === 403)) {
			throw new ProviderUnavailableError(provider.name, provider.model, err.status, err.message);
		}
		throw err;
	}
}
