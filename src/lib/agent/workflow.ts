...
import { callProvider, emitProgress, executeToolCall, SYSTEM_PROMPT, type DeveloperSummary } from './steps/investigate';
...
export async function investigateGitHubUser(username: string) {
	...
	for (let turn = 0; turn < MAX_TURNS; turn++) {
		...
		for (let pi = 0; pi < providers.length; pi++) {
			...
			await emitProgress(
				'synthesis',
				pi === 0 ? 'asking' : 'switching',
				{ vendor: p.displayName, model: p.model },
			);
			...
			try {
				...
				if (isUnavailable) {
					...
					await emitProgress('synthesis', 'providerUnavailable', {
